        ...
        if sf == 1:
            ...
            if self.first == True:
                ...
                if self.fileSize > 1073741824:
                    logger.info(
                        "The size of file to be received is {:.3} GB".format(
                            self.fileSize / 1073741824))
                elif self.fileSize > 1048576:
                    logger.info(
                        "The size of file to be received is {:.3} MB".format(
                            self.fileSize / 1048576))
                else:
                    logger.info(
                        "The size of file to be received is {:.3} KB".format(
                            self.fileSize / 1024))
                self.NextSeqNum = seqNum + len(data)
                ...
                if progress < self.progress:
                    logger.info('Received {0}%'.format(
                        (self.progress - 1) * 5))
                    speed = self.count * self.MSS / (
                        time.time() - self.lastTime)
                    if speed > 1073741824:
                        logger.info("Speed: {:.4} GB/s".format(
                            speed / 1073741824))
                    elif speed > 1048576:
                        logger.info("Speed: {:.4} MB/s".format(
                            speed / 1048576))
                    else:
                ...
                i = 0
                while i < len(
                        self.RcvBuffer) and self.RcvBuffer[i][0] < seqNum:
                    i += 1
                ...
                if len(self.RcvBuffer) == 0 or i == len(
                    ...
                    while i < len(
                        ...
                        if self.RcvBuffer[i][2] == 2:
                            self.file.close()
                            logger.info('File received from {0}'.format(
                                self.clientAddress))
                            # ZYD : update here for encapsulation
                            finishFlag = True
                        else:
