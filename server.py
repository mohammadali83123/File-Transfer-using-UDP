...
def userConnection(clientAddr, serverPort):
    ...
    s = " "
    logger.info("Start a new thread for : " + str(clientAddr) + " at port " +
                str(serverPort))
    ...
    # the file exists in server
        ...
        logger.info("Check file exist :{0}".format(clientFileName))
    ...
    if clientCommand == "lsend":
        ...
        UDPReceiver.getFile(serverPort, clientFileName)
    # client gets, server sends
    else:
        logger.info("Send " + clientFileName + " to " + str(clientAddr[0]))
        UDPSender.sendFile((clientAddr[0], serverPort), clientFileName)
    ...
    while True:
        ...
        if clientData == "HAND SHAKE 1":
            ...
            if clientData == "HAND SHAKE 3":
                ...
                _thread.start_new_thread(userConnection,
                                         (clientAddr, LISTEN_PORT + userCount))
            else:
